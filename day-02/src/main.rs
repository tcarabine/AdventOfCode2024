...
fn is_safe(readings: &Vec<i32>) -> bool {
  ...
  return readings.windows(2).all(|pair| is_stable(&pair[0],&pair[1],&comp));
}
...
fn can_be_made_safe(readings: &Vec<i32>) -> bool {
  for (remove, _) in readings.iter().enumerate() {
    ...
    if is_safe(&amended) {return true;}
  }
  return false;
}
...
fn main() {
    ...
    let numbers: Vec<Vec<i32>> = data.iter().map(|line| 
      line.split_whitespace().map(|x| x.parse::<i32>().unwrap()).collect()
    ).collect();

    let part1 = numbers.iter().filter(|readings| is_safe(*readings)).collect::<Vec<&Vec<i32>>>().len();
